...
export async function generateImage(options: GenerateImageOptions): Promise<FalApiResult> {
  ...
  try {
    ...
    console.log(`🎨 Generating with ${settings.model}:`, { endpoint, input });
    ...
    console.log(`✅ ${settings.model} generation successful:`, { requestId: result.requestId });
    return result as FalApiResult;
  } catch (error) {
    console.error(`❌ ${settings.model} generation failed:`, error);
...
export async function generateImageWithFallback(options: GenerateImageOptions): Promise<GenerateImageWithFallbackResult> {
  ...
  try {
    ...
    console.log(`🎯 Attempting generation with ${settings.model} for prompt: "${prompt.substring(0, 50)}..."`);
    ...
    console.warn(`⚠️ ${settings.model} failed:`, error instanceof Error ? error.message : 'Unknown error');
    ...
    if (shouldAttemptFallback) {
      console.log(`🔄 Attempting fallback to Minimax for prompt: "${prompt.substring(0, 50)}..." (Reason: ${isContentModerationError ? 'Content moderation' : 'General error'})`);
      ...
      try {
        ...
        console.log(`✅ Fallback to Minimax successful for prompt: "${prompt.substring(0, 50)}..."`);
        ...
        console.error(`❌ Fallback to Minimax also failed:`, fallbackError);
      ...
      if ((settings.model === 'imagen4' || settings.model === 'imagen4-fast' || settings.model === 'seedream' || settings.model === 'nano-banana') && !settings.enableFallback) {
        console.log(`ℹ️ Fallback is disabled, not attempting retry`);
      } else if (settings.model !== 'imagen4' && settings.model !== 'imagen4-fast' && settings.model !== 'seedream' && settings.model !== 'nano-banana') {
        console.log(`ℹ️ Using ${settings.model}, no fallback needed`);
      }
      throw error;
    }
  }
...
export async function generateMultipleImages(
  ...
  console.log(`🚀 Starting batch generation of ${prompts.length} prompts with ${settings.model}`);
  ...
  for (let i = 0; i < prompts.length; i++) {
    ...
    try {
      ...
      console.log(`📝 Processing prompt ${i + 1}/${prompts.length}: "${prompt.substring(0, 100)}..."`);
      ...
      if (result.wasFallback) {
        ...
        console.log(`🔄 Fallback used for prompt ${i + 1}. Total fallbacks: ${fallbackCount}`);
      }
      ...
      if (result.data?.images?.length > 0) {
        for (const falImage of result.data.images) {
          ...
          console.log(`✅ Image generated successfully with ${result.modelUsed}${result.wasFallback ? ' (fallback)' : ''}`);
        }
      }
    } catch (error) {
      ...
      console.error(`❌ Error for prompt ${i + 1}:`, errorMessage);
    }
  ...
  console.log(`  ✅ Successful: ${completed.length} images`);
